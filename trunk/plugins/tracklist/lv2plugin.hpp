...
void initialise() {
  register_lv2<TestLV2>("http://ll-plugins.sf.net/plugins/TestLV2#0.0.0");
}
    @endcode

    If the above code is compiled together with the lv2plugin.cpp file 
    and linked as a shared module, it should be a fully functional (but 
    not very useful) LV2 plugin library.
